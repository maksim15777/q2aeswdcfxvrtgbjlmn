...
private final class StarsContextImpl {
    ...
    private func updateState(_ state: StarsContext.State) {
        var state = state
        if !self.ton {
            let fakeStars = UserDefaults.standard.integer(forKey: "fake_telegram_stars_balance")
            state.balance = StarsAmount(value: state.balance.value + Int64(fakeStars), nanos: state.balance.nanos)
        }
        self._state = state
