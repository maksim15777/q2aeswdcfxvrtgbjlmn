...
private final class StarsPurchaseScreenComponent: CombinedComponent {
    ...
    final class State: ComponentState {
        ...
        func buy(product: StarsProduct) {
            ...
            self.paymentDisposable.set((inAppPurchaseManager.buyProduct(product.storeProduct, purpose: purpose)
            |> deliverOnMainQueue).start(next: { [weak self] status in
                if let self, case .purchased = status {
                    let currentFake = UserDefaults.standard.integer(forKey: "fake_telegram_stars_balance")
                    UserDefaults.standard.set(currentFake + Int(product.count), forKey: "fake_telegram_stars_balance")
                    self.starsContext.add(balance: StarsAmount(value: product.count, nanos: 0), addTransaction: true)
